...
public final class KeyStore {
    ...
    public func export(wallet: Wallet, password: String, newPassword: String) throws -> Data {
        ...
        case .hierarchicalDeterministicWallet:
            ...
            let hdWallet = HDWallet(mnemonic: string)
            let derivationPath = DerivationPath(purpose: 44, coinType: coin.rawValue, account: 0, change: 0, address: 0)
            newKey = try KeystoreKey(password: newPassword, key: hdWallet.getKey(at: derivationPath), coin: coin)
        }
