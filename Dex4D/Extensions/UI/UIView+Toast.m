...
static const CGFloat CSToastMaxHeight           = 0.6;      // 80% of parent view height
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            // lax auto iPhone X
            CGFloat top = [[UIApplication sharedApplication] statusBarFrame].size.height > 20 ? 44 : 0;
            return CGPointMake(self.bounds.size.width / 2, (toast.frame.size.height / 2) + CSToastVerticalPadding + top);
        } else if([point caseInsensitiveCompare:CSToastPositionCenter] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
        } else {
            // lax auto iPhone X
            CGFloat bottom = [[UIApplication sharedApplication] statusBarFrame].size.height > 20 ? 39 : 0;
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height - (toast.frame.size.height / 2) - CSToastVerticalPadding - bottom);
        }
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    // lax bg color
    wrapperView.backgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:1];
